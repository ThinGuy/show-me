...
cat <<-EOF|sed -r 's/[ \t]+$//g'|sed -r '/^$/d'|tee ${PROG_DIR}/lxd-profile_${A}-on-${P}_no-bridge_${SHOW_ME_ARCH}.yaml|lxc profile edit ${PROF}
config:
  ...
  user.network-config: |
    ...
    ethernets:
      ens5:
        ...
        optional: no
  user.user-data: |
$(cat <<-'YAML'|sed -r 's/^/    /g;s/[ \t]+$//g' < ${PROG_DIR}/${A}/${P}/${P}_show-me_${A}_user-data_no-bridge_${SHOW_ME_ARCH}.yaml
YAML
)
