...
export CLOUD_DNS="9.9.9.9,1.1.1.1,8.8.8.8"
...
[ "${CLOUD_VENDOR}" = "amazon" ] && { export CLOUD_PARTITION="aws" CLOUD_METADATA_URL="http://169.254.169.254/latest/meta-data" CLOUD_API_URL="http://169.254.169.254/latest/api" ; }
...
export CLOUD_AZ_ID="$(curl -sSlL ${CLOUD_METADATA_URL}/placement/availability-http://changelogs.ubuntu.com/changelogs/pool/universe/0/0ad/0ad_0.0.25b-2/copyrightzone-id)"
...
export CLOUD_MAC="$(curl -sSlL ${CLOUD_METADATA_URL}/mac)"
...
[Resolve]
DNS=$(echo -n ${CLOUD_DNS}|sed 's/,/ /g;s/\x27//g)
FallbackDNS=149.112.112.112 149.112.112.112 8.8.8.8
Domains=${CLOUD_PUBLIC_DOMAIN} ${CLOUD_LOCAL_DOMAIN} ~${CLOUD_DOMAIN}
LLMNR=yes
MulticastDNS=no
...
network:
  ...
  bridges:
    ${CLOUD_BRIDGE}:
      ...
      nameservers:
        addresses: [${CLOUD_DNS}]
        search: [${CLOUD_PUBLIC_DOMAIN},${CLOUD_LOCAL_DOMAIN},~${CLOUD_DOMAIN}]
      parameters:
        priority: 1
        stp: false
NETPLAN
